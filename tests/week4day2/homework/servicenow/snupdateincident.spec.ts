...
test.describe(`ServiceNow Incident handling`, async () => {
    ...
    test(`TC001 - Update an Incident`,async ({page,context}) => {
        ...
        const urgencyLabel = iframe.locator("select[id='incident.urgency']")
        await urgencyLabel.selectOption("1")

        const statedd = iframe.locator("select[id='incident.state']")
        await statedd.selectOption("2")

        const worknotes = iframe.locator("[id='activity-stream-textarea']")
        await worknotes.fill("Updated the Urgency and State of the Incident")

        const updatebtn = iframe.locator("button[id='sysverb_update']")
        await updatebtn.click()

        await page.waitForTimeout(5000)

        const stateLabel = iframe.locator("td[class='vt']").nth(5)
        expect(stateLabel).toHaveText("In Progress")
